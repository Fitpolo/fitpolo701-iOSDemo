...
@implementation fitpolo701LogManager
...
+ (void)writeCommandToLocalFile:(NSArray *)dataList
    ...
    NSString *sourceInfo = (source == fitpolo701DataSourceAPP ? @"app:" : @"device:");
...
+ (NSData *)readCommandDataFromLocalFile{
    ...
    if (!fitpolo701ValidStr(fileString)) {
        return nil;
    }
...
+ (NSInteger)getWeekInfoWithDateString:(NSString *)dateString{
    if (!fitpolo701ValidStr(dateString)) {
        return 0;
    }
    NSArray * dateArr = [dateString componentsSeparatedByString:@"-"];
    
    if (!fitpolo701ValidArray(dateArr)
        || dateArr.count != 3) {
        return 0;
    }
